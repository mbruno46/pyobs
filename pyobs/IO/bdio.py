...
class bdio_file(binary_file):
    ...
    def read_bdio_header(self):
        ...
        self.header['dirinfo'] = [(hdr[2] & int('ffc00000',16))>>22, (hdr[2] & int('003fffff',16))]
    ...
    def parse_record_content(self, uinfo, fmt, rlen):
        if fmt == bdio_const.BDIO_BIN_GENERIC:
            if uinfo == 7:
                ...
                pyobs.assertion(self.read(dtypes.INT32) == self.BDIO_HASH_MAGIC_S,'Failed checking HASH_MAGIC')
                md5 = ['%02hX' % int.from_bytes(self.read_binary(1),'little') for _ in range(16)]
                return ''.join(md5)
            else:
            ...
            if little:
                return self.read(dtypes.FLOAT64, rlen//dtypes.FLOAT64.itemsize).byteswap()
            else:
...
def load(fname):
    ...
    for i in range(len(f.records)):
        r = f.records[i]
        if (r['uinfo']==7) and (f.records[i-1]['fmt']==bdio_const.BDIO_BIN_GENERIC):
            pyobs.assertion(f.records[i-1]['content'] == r['content'], 'MD5 checksums do not match')
        elif r['uinfo']==2:
