...
class variance:
    def __init__(self,n,g,Stau,D,k,fold=False):
        ...
        idx = numpy.array(self.x,dtype=numpy.int)**(1 if D==1 else 2)
    ...
    def stat_relerr(self,r,a):
        return numpy.sqrt(2.*self.OmegaD/(self.D*self.N[a]) * numpy.array(r)**self.D)
    ...
    if ismf:
        ...
        for ik in keys:
            if numpy.any(x.delta[ik].lat != lat):
                raise pyobs.PyobsError('Lattice does not match among different replicas of same ensemble')
