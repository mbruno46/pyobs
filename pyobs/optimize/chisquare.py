...
class chisquare:
    ...
    def grad(self,y,pdict):
        ...
        for i in range(self.n):
            self.de[i,:] = numpy.array(self.df(*self.x[i,:], *self.p)) # N x Na matrix
...
class mfit:
    ...
    def __call__(self, yobs, p0=None, min_search=None):
        ...
        dcsq  = lambda x: sum([self.csq[i].grad(yobs[i].mean,self.pdict) for i in range(len(yobs))])
        ddcsq = lambda x: sum([self.csq[i].hess(yobs[i].mean,self.pdict) for i in range(len(yobs))])
